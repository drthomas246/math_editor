 ...
 */
export function getSugakuJitateCapabilities() {
  const session = webMcpSession.getSnapshot();
  const availableTools = ["sujita_get_capabilities", "sujita_validate_import"];
  if (session.directImportAvailable) availableTools.push("sujita_import_worksheet");
  return {
    app: "sujita" as const,

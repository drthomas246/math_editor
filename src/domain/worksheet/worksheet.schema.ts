...
const FORBIDDEN_LATEX_COMMAND_PATTERN =
  /\\(?:href|url|includegraphics|html(?:Class|Id|Style|Data)|css(?:Id|Class)|class|style)\b/iu;
...
function validateRichTextLimits(
  document: { content: readonly unknown[] },
  context: z.RefinementCtx,
): void {
  let nodeCount = 1;
  const stack: Array<{ node: unknown; depth: number }> = document.content.map((node) => ({
    node,
    depth: 2,
  }));
...
function validateTableData(
  ...
  const widthTotal = table.columnWidthsPercent.reduce((total, width) => total + width, 0);
  ...
  if (occupied.some((row) => row.some((cell) => !cell))) {
    context.addIssue({
      code: "custom",
...
export const TableBlockSchema = TableBlockBaseSchema.superRefine((table, context) => {
...
export const RichTableNodeSchema = RichTableNodeBaseSchema.superRefine((node, context) => {
...
export const ImageBlockSchema = z
  ...
  .meta({
    title: "ImageBlock",
    description:
      "画像本体を含まない参照情報。assetIdはIndexedDB assetsまたはバックアップassetsのidを参照する。",
  });
...
function validateWorksheetHeaderTitle(
  worksheet: { title: string; header: { title: string } },
  context: z.RefinementCtx,
): void {
...
function visitRichTextDocument(
  document: { content: readonly unknown[] },
  path: EntityPath,
  worksheetId: string,
  registerId: (id: string, path: EntityPath) => void,
  registerAssetReference: (assetId: string, worksheetId: string, path: EntityPath) => void,
): void {
  const visitNode = (node: unknown, nodePath: EntityPath): void => {
    ...
    if (value.type === "richTable" && value.attrs) {
      ...
      if (Array.isArray(rows)) {
        visitTableRows(
          rows as Array<z.infer<typeof TableRowSchema>>,
          [...nodePath, "attrs", "rows"],
          worksheetId,
          registerId,
    ...
    if (Array.isArray(value.content)) {
      value.content.forEach((child, index) => visitNode(child, [...nodePath, "content", index]));
    }
  };

  document.content.forEach((node, index) => visitNode(node, [...path, "content", index]));
}
...
function visitWorksheet(
  ...
  worksheet.problems.forEach((problem, problemIndex) => {
    const problemPath = [...path, "problems", problemIndex];
    registerId(problem.id, [...problemPath, "id"]);

    problem.contents.forEach((content, contentIndex) => {
      ...
      switch (content.type) {
        ...
        case "answerArea":
          visitRichTextDocument(
            content.answerArea.document,
            [...contentPath, "answerArea", "document"],
            worksheet.id,
            registerId,
            registerAssetReference,
          );
          visitRichTextDocument(
            content.answerArea.answerDocument,
            [...contentPath, "answerArea", "answerDocument"],
            worksheet.id,
            registerId,
            registerAssetReference,
          );
        ...
        case "subQuestionGroup":
          content.items.forEach((item, itemIndex) => {
            ...
            if (item.answerArea) {
              visitRichTextDocument(
                item.answerArea.document,
                [...itemPath, "answerArea", "document"],
                worksheet.id,
                registerId,
                registerAssetReference,
              );
              visitRichTextDocument(
                item.answerArea.answerDocument,
                [...itemPath, "answerArea", "answerDocument"],
                worksheet.id,
                registerId,
                registerAssetReference,
              );
...
function validateBackupFile(file: BackupFileValue, context: z.RefinementCtx): void {
  ...
  const assetReferences = new Map<
    string,
    Array<{ worksheetId: string; path: EntityPath }>
  >();

  const registerId = (id: string, path: EntityPath): void => {
  ...
  worksheets.forEach((worksheet, index) => {
    visitWorksheet(
      worksheet,
      file.kind === "single" ? ["worksheet"] : ["worksheets", index],
      registerId,
      registerAssetReference,
    );
  });
...
export const MathWorksheetSingleFileSchema = MathWorksheetSingleFileObjectSchema.superRefine(
  validateBackupFile,
);

export const MathWorksheetArchiveSchema = MathWorksheetArchiveObjectSchema.superRefine(
  validateBackupFile,
);

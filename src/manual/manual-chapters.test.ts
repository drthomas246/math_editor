...
describe("manual chapters", () => {
  it("12章を定義順かつ空でない本文で提供する", () => {
    expect(MANUAL_CHAPTERS).toHaveLength(12);
    expect(new Set(MANUAL_CHAPTERS.map((chapter) => chapter.slug))).toHaveProperty("size", 12);
  ...
  it("章を解決して前後章を返す", () => {
    ...
    expect(getAdjacentManualChapters("troubleshooting")).toEqual({ previous: MANUAL_CHAPTERS[9], next: MANUAL_CHAPTERS[11] });
    expect(getAdjacentManualChapters("version-and-license")).toEqual({ previous: MANUAL_CHAPTERS[10] });
  });
});

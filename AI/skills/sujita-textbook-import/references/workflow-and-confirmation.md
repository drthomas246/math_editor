...
- PDFと処理に必要な内容はすうがく仕立て外部のSkill実行環境へ渡される。
- すうがく仕立て本体のブラウザ内ローカル保存とは処理経路が異なる。
- 利用者がPDFの利用権限を確認し、Skillは許諾取得や法的判断を代行しない。
- 最終JSONには採用内容と採用図版だけを含み、PDF本体、範囲外本文、棄却Crop、OCR中間結果を含めない。
- PDF解析・Draft構築には外部AI API、すうがく仕立て API、外部MCPを使わない。完成確認とBuilder / Validator成功後に、対象のすうがく仕立てページへ完成JSONだけを渡してWebMCP検証を行い、利用者がそのページセッションでAI連携を許可した場合は新規Worksheetとして直接追加することがある。許可しない場合やWebMCPを使えない場合はJSONファイルを手動で取り込める。

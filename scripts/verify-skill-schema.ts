...
const rootSchemaPath = fileURLToPath(
  new URL("../schemas/math-worksheet.schema.json", import.meta.url),
);
const skillSchemaPath = fileURLToPath(
  new URL(
    "../AI/skills/math-editor-textbook-import/schemas/math-worksheet.schema.json",
    import.meta.url,
  ),
);
const manifestPath = fileURLToPath(
  new URL(
    "../AI/skills/math-editor-textbook-import/schemas/schema-manifest.json",
    import.meta.url,
  ),
);
const validatorPath = fileURLToPath(
  new URL(
    "../AI/skills/math-editor-textbook-import/scripts/validate_math_worksheet.mjs",
    import.meta.url,
  ),
);
...
const VALIDATOR_METADATA_PATTERN =
  /^\/\* math-editor-validator-metadata (\{[^\r\n]+\}) \*\/$/mu;
...
function parseJson(contents: string, label: string, errors: string[]): unknown {
  try {
    ...
    errors.push(
      `${label}をJSONとして読み取れません: ${error instanceof Error ? error.message : String(error)}`,
    );
    return null;
  }
}
...
async function readValidatorSchemaVersion(errors: string[]): Promise<unknown> {
  const result = await new Promise<{ stdout: string; stderr: string }>((resolve, reject) => {
    ...
    errors.push(
      `Validatorを実行できません: ${error instanceof Error ? error.message : String(error)}`,
    );
    return null;
  });
...
expectEqual(
  ...
  "Validator実行結果のschemaVersionがCURRENT_SCHEMA_VERSIONと一致しません",
  errors,
);

if (rootSchema !== skillSchema) {
  errors.push(
    "ルートSchemaとSkill同梱Schemaが一致しません。Schema生成後にSkill側へコピーしてください。",
  );
}
...
if (!isObject(manifest)) {
  ...
  expectEqual(
    ...
    CURRENT_SCHEMA_VERSION,
    "manifest.schemaVersionがCURRENT_SCHEMA_VERSIONと一致しません",
    errors,
  );
  expectEqual(manifest.source, EXPECTED_SOURCE, "manifest.sourceが不正です", errors);
  expectEqual(
    manifest.generatedSchema,
    EXPECTED_GENERATED_SCHEMA,
    "manifest.generatedSchemaが不正です",
    errors,
  );
  expectEqual(
    typeof manifest.sha256 === "string" ? manifest.sha256.toUpperCase() : manifest.sha256,
    schemaSha256,
    "manifest.sha256がルートSchemaのSHA-256と一致しません",
    errors,
  );
  ...
  if (!isObject(manifest.validator)) {
    ...
    expectEqual(
      ...
      "manifest.validator.pathが不正です",
      errors,
    );
    expectEqual(
      manifest.validator.schemaVersion,
      CURRENT_SCHEMA_VERSION,
      "manifest.validator.schemaVersionがCURRENT_SCHEMA_VERSIONと一致しません",
      errors,
    );
    expectEqual(
      typeof manifest.validator.schemaSha256 === "string"
        ? manifest.validator.schemaSha256.toUpperCase()
        : manifest.validator.schemaSha256,
      schemaSha256,
      "manifest.validator.schemaSha256がルートSchemaのSHA-256と一致しません",
      errors,
    );
    expectEqual(
      typeof manifest.validator.sha256 === "string"
        ? manifest.validator.sha256.toUpperCase()
        : manifest.validator.sha256,
      validatorSha256,
      "manifest.validator.sha256がValidatorのSHA-256と一致しません",
      errors,
    );
  }
}
...
if (!metadataMatch?.[1]) {
  ...
  if (!isObject(metadata)) {
    ...
    expectEqual(
      ...
      CURRENT_SCHEMA_VERSION,
      "Validator metadataのschemaVersionがCURRENT_SCHEMA_VERSIONと一致しません",
      errors,
    );
    expectEqual(
      typeof metadata.schemaSha256 === "string"
        ? metadata.schemaSha256.toUpperCase()
        : metadata.schemaSha256,
      schemaSha256,
      "Validator metadataのschemaSha256がルートSchemaのSHA-256と一致しません",
      errors,
    );
  }
}
...
if (errors.length > 0) {
  ...
  console.log(
    `AI SkillのSchema同梱物は同期されています（schemaVersion: ${CURRENT_SCHEMA_VERSION}, schema SHA-256: ${schemaSha256}）。`,
  );
}

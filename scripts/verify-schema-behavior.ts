...
expectInvalid(oversizedFloatImageRefFile, "66%以上の回り込みRichText imageRefを拒否する必要がある");
...
assert.equal(TableCellRichTextDocumentSchema.safeParse(nestedTableCellDocument).success, false, "表セル内のrichTableを拒否する必要がある");
...
expectInvalid(legacyBlockTextSizeFile, "RichTextBlock全体へ置く旧textSizeを拒否する必要がある");
...
normalTextSizeMarkFile.worksheet.problems[0].contents[0].document.content[0].content[1].marks[0].attrs.size =
    "normal";
expectInvalid(normalTextSizeMarkFile, "標準文字サイズはtextSize markを残さず正規化する必要がある");
...
assert.equal(WorksheetSchema.safeParse(problemLimitWorksheet).success, false, "201問のWorksheetを拒否する必要がある");
...
assert.equal(MathWorksheetArchiveSchema.safeParse(archiveAtLimit).success, true, "2,000件の全体バックアップを受理する必要がある");
archiveAtLimit.worksheets.push(createWorksheet("archive-worksheet-2001"));
assert.equal(MathWorksheetArchiveSchema.safeParse(archiveAtLimit).success, false, "2,001件の全体バックアップを拒否する必要がある");
...
assert.equal(AssetRecordSchema.safeParse({
    id: "asset-1",
    worksheetId: "worksheet-1",
    mimeType: "image/png",
...
assert.equal(AssetRecordSchema.safeParse({
    ...
    height: 100,
    createdAt: timestamp,
}).success, false, "IndexedDB AssetRecordでもSVGを拒否する必要がある");
const generatedSchemaPath = fileURLToPath(new URL("../schemas/math-worksheet.schema.json", import.meta.url));
